...
test("browser-authenticated billing actions surface owner bypass notices", async (t) => {
  ...
  try {
    ...
    try {
      ...
      await page.getByTestId("billing-notice").waitFor({ state: "visible", timeout: 30_000 });
      assert.match((await page.getByTestId("billing-notice").textContent()) || "", /Owner account detected. Trial activation is not required./i);

      await page.getByTestId("billing-open-checkout").click();
      await page.getByTestId("billing-notice").waitFor({ state: "visible", timeout: 30_000 });
      assert.match((await page.getByTestId("billing-notice").textContent()) || "", /Owner account detected. Checkout bypass is active./i);

      await page.getByTestId("billing-manage-subscription").click();
      await page.getByTestId("billing-notice").waitFor({ state: "visible", timeout: 30_000 });
      assert.match((await page.getByTestId("billing-notice").textContent()) || "", /Owner account detected. Stripe portal is not required./i);
