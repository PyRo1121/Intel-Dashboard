...
test("readTelegramCollectorRuntimeConfig normalizes env values", () => {
  const config = readTelegramCollectorRuntimeConfig({
    ...
    COLLECTOR_SELF_URL: "https://intel-dashboard-telegram-collector.latham.workers.dev",
  ...
  assert.equal(config.selfUrl, "https://intel-dashboard-telegram-collector.latham.workers.dev");
