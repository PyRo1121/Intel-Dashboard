...
async function handleAdminCrmSummary(args: {
  ...
  if (isCrmStripeLiveEnabled(args.env) && trimString(args.env.STRIPE_SECRET_KEY)) {
    const stripeResult = await fetchStripeLiveCrmSummary(args.env);
    stripeLive = stripeResult.ok
