        ...
        >
          <section class="surface-card mb-4 space-y-3 p-4">
            <div class="flex items-center justify-between gap-3">
              ...
              <button
                ...
                class="rounded-sm border border-violet-400/20 bg-violet-500/10 px-3 py-2 text-sm text-violet-200 disabled:opacity-50"
              >
                {savingControls() ? "Saving..." : "Save controls"}
              </button>
            </div>
            <div class="grid gap-3 md:grid-cols-2 xl:grid-cols-3 text-sm">
              <label class="flex items-center justify-between gap-3 rounded-sm border border-white/[0.08] bg-black/20 px-3 py-2 text-zinc-300">
              ...
              <label class="flex items-center justify-between gap-3 rounded-sm border border-white/[0.08] bg-black/20 px-3 py-2 text-zinc-300">
                ...
                <select
                  ...
                  class="rounded-sm border border-white/[0.08] bg-black/40 px-2 py-1 text-zinc-100"
                >
          ...
          <section class="surface-card mb-4 flex flex-wrap items-center justify-between gap-3 p-4">
            <div class="flex items-center gap-3">
              <span class="rounded-sm border border-amber-400/20 bg-amber-500/10 px-3 py-2 text-sm font-medium text-amber-200">
                Unread {unreadCount()}
              </span>
                ...
                <For each={STATES}>
                  {(entryState) => (
                    <button
                      ...
                      class={`rounded-sm border px-3 py-1.5 text-[12px] ${state() === entryState ? "border-blue-400/40 bg-blue-500/15 text-blue-200" : "border-white/[0.08] bg-black/20 text-zinc-500"}`}
                    >
                      {entryState}
                    </button>
                  )}
            ...
            <button
              ...
              class="rounded-sm border border-white/[0.08] bg-black/20 px-3 py-2 text-sm text-zinc-300 disabled:opacity-50"
            >
                ...
                {(item) => (
                  <article class="surface-card p-4">
                    <div class="mb-2 flex flex-wrap items-center gap-2 text-[11px] text-zinc-400">
                      ...
                      <span class="rounded-none border border-amber-400/20 bg-amber-500/10 px-2 py-0.5 text-amber-200">{item.type}</span>
                      ...
                      <span class="rounded-none border border-amber-400/20 bg-amber-500/10 px-2 py-0.5 text-amber-200">{item.matchedPreference}</span>
                    ...
                    <div class="mt-3 flex flex-wrap gap-2">
                      <a href={item.link} target="_blank" rel="noopener noreferrer" class="rounded-sm border border-blue-400/20 bg-blue-500/10 px-3 py-2 text-xs text-blue-200 no-underline">
                      ...
                      <Show when={!item.readAt}>
                        <button
                          ...
                          class="rounded-sm border border-white/[0.08] bg-black/20 px-3 py-2 text-xs text-zinc-300 disabled:opacity-50"
                        >
