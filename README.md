...
1. `public-browser`
   - local built-app browser contract for the public auth-entry surface and 404 route
   - covers the `/login` and `/signup` UI contract plus public-route diagnostics without depending on Cloudflare challenge behavior
   - this lane intentionally avoids Cloudflare challenge variability; the broader `e2e-production` workflow remains the production/live sweep
2. `owner-admin-browser`
   - hardened `/billing`, `/crm`, and related owner/admin flows
