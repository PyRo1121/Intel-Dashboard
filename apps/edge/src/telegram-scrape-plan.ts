...
export function resolveTelegramScrapePlan(args: {
  ...
  const prioritized = [...args.channels].sort((left, right) => right.subscriberValueScore - left.subscriberValueScore);
  ...
  return {
    channels: selected.length > 0 ? [...hotChannels, ...selected] : args.channels,
    slot,
    slots,
  };
}
